...
public class ArbolBinarioOrdenado<T extends Comparable<T>>
    ...
    private class Iterador implements Iterator<T> {
        ...
        @Override public T next() {
            ...
            if (vertice.derecho != null) {
                Vertice temporal = vertice.derecho;
                pila.mete(temporal);

                while (( temporal =  temporal.izquierdo) != null)
                    pila.mete( temporal);
            }
	...
	if(vertice.elemento.compareTo(actual.elemento) <= 0){
	    ...
	    } else  agrega(vertice, actual.derecho);
   ...
   private Vertice maximoEnSubarbol(Vertice vertice){
    ...
    protected Vertice intercambiaEliminable(Vertice vertice) {
       	Vertice verticeIntercambio = maximoEnSubarbol(vertice.izquierdo);
	T temporal = verticeIntercambio.elemento;
	verticeIntercambio.elemento = vertice.elemento;
        vertice.elemento = temporal;

        return verticeIntercambio;
    }
	...
	Vertice hijo = vertice.izquierdo != null ?
    ...
    public void giraDerecha(VerticeArbolBinario<T> vertice) {
        ...
        Vertice padre = verticeActual.padre;
        hijo.padre = padre;

        if (!verticeActual.hayPadre())
            raiz = hijo;
	
        else {

            if (esHijoIzquierdo(verticeActual))
    ...
    public void giraIzquierda(VerticeArbolBinario<T> vertice) {
        ...
        if (!verticeActual.hayPadre())
            ...
            if (esHijoDerecho(verticeActual))
                padre.derecho = hijo;
            else
                padre.izquierdo = hijo;
